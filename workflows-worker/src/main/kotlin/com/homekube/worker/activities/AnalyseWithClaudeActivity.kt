...
class AnalyseWithClaudeActivity {
    ...
    fun execute(input: AnalysisInput): AnalysisResult {
        ...
        return try {
            ...
            if (jsonMatch == null) {
                log.warnf("[Claude] No JSON found in response, defaulting to no-notify: %s", claudeText.take(200))
            ...
            log.warnf("[Claude] Failed to parse JSON response, defaulting to no-notify: %s", responseBody.take(200))
