...
export async function summariseEconomistWithClaude(articles: EconomistArticle[]): Promise<string> {
    ...
    const promptString = `You are a news digest assistant. Summarise the following articles from The Economist into a WhatsApp-friendly digest.
...
${articleText}`;
    ...
    if (!digest || digest.trim() === '') {
        throw new Error('Claude returned empty Economist digest');
    }
