<script lang="ts">
    ...
    async function fetchJobs() {
        ...
        try {
            ...
            if (!token) return;
            const res = await fetch(`${config.workflows.baseUrl}/api/scraper/jobs`, {
    ...
    async function saveEdit() {
        ...
        try {
            ...
            const res = await fetch(`${config.workflows.baseUrl}/api/scraper/jobs/${editingJob.id}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
    ...
    async function toggleEnabled(job: Job) {
        try {
            const token = await getFreshToken();
            if (!token) return;
            const res = await fetch(`${config.workflows.baseUrl}/api/scraper/jobs/${job.id}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
    ...
    async function triggerJob(job: Job) {
        ...
        try {
            const token = await getFreshToken();
            if (!token) return;
            const res = await fetch(`${config.workflows.baseUrl}/api/scraper/jobs/${job.id}/trigger`, {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${token}` },
            });
            const data = await res.json();
    ...
    async function toggleRuns(jobId: string) {
        ...
        try {
            const token = await getFreshToken();
            if (!token) return;
            const res = await fetch(`${config.workflows.baseUrl}/api/scraper/jobs/${jobId}/runs?limit=10`, {
                headers: { 'Authorization': `Bearer ${token}` },
            });
            const data = await res.json();
            if (res.ok) {
