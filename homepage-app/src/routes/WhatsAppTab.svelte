<script lang="ts">
    ...
    async function fetchStatus() {
        try {
            ...
            if (!token) return;
            const res = await fetch(`${config.whatsapp.baseUrl}/api/status`, {
    ...
    async function fetchQr() {
        try {
            const token = await getFreshToken();
            if (!token) return;
            const res = await fetch(`${config.whatsapp.baseUrl}/api/qr`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            const data = await res.json();
            if (data.qr) {
    ...
    async function linkViaQr() {
        ...
        try {
            const token = await getFreshToken();
            if (!token) return;
            const res = await fetch(`${config.whatsapp.baseUrl}/api/link`, {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            const data = await res.json();
    ...
    async function linkViaPairingCode() {
        ...
        try {
            const token = await getFreshToken();
            if (!token) return;
            const res = await fetch(`${config.whatsapp.baseUrl}/api/register`, {
                method: 'POST',
                headers: {
    ...
    async function disconnect() {
        ...
        try {
            const token = await getFreshToken();
            if (!token) return;
            await fetch(`${config.whatsapp.baseUrl}/api/disconnect`, {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${token}` }
            });
    ...
    async function sendMessage() {
        ...
        try {
            ...
            const res = await fetch(`${config.whatsapp.baseUrl}/api/send`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
