use text_to_speech::auth;
use text_to_speech::cleanup;
use text_to_speech::handlers;
use text_to_speech::inference;
use text_to_speech::nats_producer::NatsProducer;
use text_to_speech::state::{AppState, JwksCache};
use text_to_speech::ws_handler;
...
async fn main() {
    ...
    let kokoro_model = match inference::KokoroModel::load(
        &std::env::var("KOKORO_MODEL_PATH").unwrap_or_else(|_| "/app/kokoro-v1.0.onnx".to_string()),
        &std::env::var("KOKORO_VOICES_PATH").unwrap_or_else(|_| "/app/voices-v1.0.bin".to_string()),
    ) {
        Ok(model) => {
    ...
    // Connect NATS producer (optional -- if NATS_URL not set, fall back to spawn_blocking)
    let nats_producer = match std::env::var("NATS_URL") {
        Ok(url) => match NatsProducer::new(&url).await {
            Ok(p) => {
                tracing::info!("NATS producer ready");
                Some(Arc::new(p))
            }
            Err(e) => {
                tracing::warn!(error = %e, "Failed to connect NATS producer, falling back to spawn_blocking");
                None
            }
        },
        Err(_) => {
            tracing::info!("NATS_URL not set, using spawn_blocking fallback");
            None
        }
    };
    ...
    tracing::info!("Starting TTS API server on 0.0.0.0:3000");
